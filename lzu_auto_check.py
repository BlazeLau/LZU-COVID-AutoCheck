...
def getSubmit(auToken, dailyCookie, info, FilledInfo):
    ...
    info_data = {
        ...
        "sfzx": "0",  # 是否在校(0离校，1在校)
        "sfgl": "0",  # 是否隔离(0正常，1隔离)
        "szsf": info_data['szsf'] if info_data['szsf'] else "天津市",  # 所在省份（没有打过卡或在校则为空）
        "szds": info_data['szds'] if info_data['szds'] else "市辖区",  # 所在地级市（没有打过卡或在校则为空）
        "szxq": info_data['szxq'] if info_data['szxq'] else "河东区",  # 所在县/区（没有打过卡或在校则为空）
